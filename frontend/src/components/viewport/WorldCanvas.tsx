...
import { findCablePath } from '../../render/cablePath';
...
interface WorldCanvasProps {
  ...
  /**
   * Placement validity from the engine. Supplied by the viewer so the UI cannot
   * shade a cell buildable that the engine would refuse; a permissive fallback
   * keeps the component usable without it.
   */
  checkPlacement?: (kind: MachineType | OverlayType, x: number, y: number)
    ...
    const renderFrame = (timestamp: number) => {
      ...
      if (cablePath.length > 0) {
        ...
        for (let i = 0; i < cablePath.length; i++) {
          ...
          const val = checkPlacement
              ? checkPlacement('Cable', pathCell.x, pathCell.y)
              : { valid: true, reason: '' };
        ...
        for (let y = 0; y < height; y++) {
          for (let x = 0; x < width; x++) {
            ...
            if (cell.machine && cell.derived.powerDelivered > 0) {
              ...
              const cableRoute = findCablePath(x, y, grid, width, height, demandZone);
        ...
        if (cell) {
          ...
          if (buildTool) {
            const validation = buildTool.kind === 'machine'
              ? (checkPlacement
                  ? checkPlacement(buildTool.type, cell.x, cell.y)
                  : { valid: true, reason: '' })
              : (checkPlacement
                  ? checkPlacement(buildTool.type, cell.x, cell.y)
                  : { valid: true, reason: '' });

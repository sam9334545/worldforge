...
surface correlations rather than rewarding them. We then built an agent on BDH-like principles --- a fast-weight associative
memory, zero-initialised each episode, updated by a local gradient-free Hebbian
rule, with no search and no backtracking --- and it \emph{fails}, scoring $18.2$
and losing money on eight of ten seeds. The failure is structural and worth more
than a success would have been: a per-site associative memory cannot represent
cannibalisation, because ``the next machine earns less because I already built
ten'' is a property of aggregate position, not of any cell. We did not run a
Dragon Hatchling model, and say so throughout. What we contribute is the
harness, the protocol, a measured cost frontier, and a concrete statement of the
state a recurrent architecture would have to carry to do well here.
\end{abstract}
...
\texttt{hebbian} & \textbf{BDH-inspired.} A fast-weight associative memory over
binned site observations, zero-initialised every episode, updated by a local
gradient-free Hebbian rule from realised outcomes. Adapts through state, never
through parameters; no backward pass, no search, no backtracking. Given no
physics --- it associates what it sees with what its machines subsequently
earned. \\
\texttt{hebbian-mwh} & The same agent with one scalar changed: its Hebbian rule
is told delivered energy is the good outcome, rather than realised value. The
ablation that separates mechanism from objective. \\
\midrule
...
\textbf{No BDH model was run.} We built an agent on BDH-like \emph{principles}
--- fast weights, state-only adaptation, local updates, no search --- but a
fast-weight vector over binned features is not a Dragon Hatchling, and its
failure is evidence about that mechanism at that scale, not about the
architecture. A real test needs the model.

\textbf{The Hebbian agent is barely tuned.} Two reward signals, one feature
binning, one learning rate. Its overbuilding may partly be poor calibration
rather than the representational limit we diagnose, and we did not run the sweep
that would separate those.

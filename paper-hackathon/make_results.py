...
""" % "\n".join(rows)
    ...
    t2 = discussion = ""
    if LLM.exists():
        # Scripted rows come from the same post-fix run as Table 1, so the two
        # tables describe one engine version. Only `lookup` moved when the
        # hydro-head defect was fixed (+4.9 on this seed); heuristic, random
        # and donothing are byte-identical either side of it.
        base = {r["agent"]: r for r in multi if r["seed"] == 1}
